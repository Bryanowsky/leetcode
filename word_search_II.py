class Solution:
    def find_words(self, board: [[str]], words: [str]) -> [str]:
        contained_words = set()

        # Implementation of the Deep First Search Algorithm
        def deep_first_search(row_index, column_index, word_, count):
            # Out of boundaries
            if row_index < 0 or row_index >= len(board) or column_index < 0 or column_index >= len(board[0]):
                return False

            # If the grid position is not equal to the word position false
            if board[row_index][column_index] != word_[count]:
                return False

            # Check if count is equal to len(word) - 1 to avoid another subsequent visit and check the last letter
            if len(word_) - 1 == count and board[row_index][column_index] == word_[count]:
                return True

            # Go to any direction (up, right, down, left)
            # To avoid visit the same letter
            actual_char = board[row_index][column_index]
            board[row_index][column_index] = '*'
            # All directions if we get a True in this or operation we are going to continue
            to_go = deep_first_search(row_index - 1, column_index, word_, count + 1) or deep_first_search(row_index,
                                                                                                          column_index + 1,
                                                                                                          word_,
                                                                                                          count + 1) or deep_first_search(
                row_index + 1, column_index, word_, count + 1) or deep_first_search(row_index, column_index - 1, word_,
                                                                                    count + 1)

            board[row_index][column_index] = actual_char
            return to_go

        for word in words:
            for row in range(len(board)):
                for column in range(len(board[0])):
                    if board[row][column] == word[0] and deep_first_search(row, column, word, 0):
                        print('Word was founded', word)
                        contained_words.add(word)
                        break
                if word in contained_words:
                    break
